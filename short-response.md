...
---
1The **`/api/auth/me`** endpoint pattern allows the database to authenticate users based on their session cookie, instead of requiring users to use their username and password on requests to update, delete, or create *(manage)* their own resources. Requiring users to do this after logging in to their account, is excessive and unnecessary. The frontend calls this endpoint in the case of a user requesting to update their password, delete their account, or create a post. The endpoint pattern returns the current user from session *(or 401: if the session is missing)*.1